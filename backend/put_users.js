 ...
 * @param {Object} event - The input event containing user information and related IDs.
...
exports.handler = async (event) => {
    console.log(event);
    const data = typeof event === 'string' ? JSON.parse(event.body) : event.body;
    const {
        id,
        username,
        email,
...
async function updateUserTable(connection, fields) {
    ...
    console.log(id, username, email, password, bio, image_url);
    const userFields = [];
    const userParams = [];
...
async function handleFollowersIds(connection, params) {
    ...
    // For following someone new
    if (follower_id !== undefined) {
        if (follower_id === null) {

            const [removedFollowers] = await connection.execute(
                'SELECT COUNT(*) as count FROM followers_ids WHERE user_id = ?', 
                [id]
            );
            await connection.execute('DELETE FROM followers_ids WHERE user_id = ?', [id]);
            ...
            await connection.execute(
                ...
                [removedFollowers[0].count, id]
            );
        } else {
            const [existingFollow] = await connection.execute(
                'SELECT * FROM followers_ids WHERE user_id = ? AND follower_id = ?',
                [id, follower_id]
            );
